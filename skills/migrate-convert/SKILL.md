...
### 1. Load the object + open a run
- Read the object row (`volume_path`, `target_uc_fqn`, `layer`, `complexity`) from `objects`.
- `start_run(object_id, step='convert', engine='genie_code')`.
...
- Emit the target (default: Lakeflow SDP; honor `config` for PySpark or Databricks SQL).
...
### 5. Record state + TODOs + audit
- For each finding, `add_todo(object_id, category, message, severity)`.
- If confidence is high and no blocker TODOs: `transition(object_id, 'converted', confidence=…,
  output_path=…)`.
- If confidence is low or a blocker TODO exists: `transition(object_id, 'needs_review',
  confidence=…, output_path=…, detail='…')` so `migrate-triage` picks it up.
- `end_run(run_id, outcome)` — `ok` / `partial` / `failed`.
