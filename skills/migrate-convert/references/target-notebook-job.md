...
- **Folder: a per-object CONTAINER folder**, hierarchical: `<output_dir>/<source_type>/<object_slug>/`
  (e.g. `.../migration-factory/alteryx/sample_sales_analytics_complex/`). Create it first; never
  dump notebooks loose in `output_dir`.
- **Naming (inside the folder): `<object_slug>__<NN>_<Tool>`** — object_slug = object_id with
  `:`→`_`, NN = ToolID, Tool = plugin short name. E.g. `sample_sales_analytics_complex__11_MultiRowFormula`.
